...
modelassump(lmer4.1)
    ...
    glmer4.1.c <- glmer(MAPY ~ urc.biomass + dummy + (1|site) + (1|year) , family = Gamma(link = "log"), data = lt)
    summary(glmer4.1.c)
...
ggplot(aes(x = per.diff, y = deltaK))+
  ...
  geom_point(aes(size = urc.biomass, fill = MAPY, color = ifelse(lt2$urc.biomass >= 668, "red", "blue")), pch = 21)+
